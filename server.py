...
class CampaignInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)
    ad_account_id: str = Field(..., description="Snapchat Ad Account ID (UUID)")
    campaign_id: Optional[str] = Field(None, description="Filter by specific Campaign ID (optional)")
...
class StatsInput(BaseModel):
    ...
    granularity: str = Field(default="TOTAL", description="TOTAL, DAY, or HOUR")
...
async def snapchat_get_ad_accounts() -> str:
    ...
    try:
        ...
        for org in orgs:
            ...
            # Step 2: get ad accounts per org
...
async def snapchat_get_campaigns(params: AdAccountInput) -> str:
    ...
    try:
        data = await snap_request(f"adaccounts/{params.ad_account_id}/campaigns")
...
async def snapchat_get_ad_squads(params: AdSquadInput) -> str:
    """List all Ad Squads (ad sets) for an account or specific campaign."""
    try:
        if params.campaign_id:
            data = await snap_request(f"campaigns/{params.campaign_id}/adsquads")
        else:
            data = await snap_request(f"adaccounts/{params.ad_account_id}/adsquads")
        squads = data.get("adsquads", [])
        if not squads:
...
async def snapchat_get_ads(params: AdAccountInput) -> str:
    """List all Ads in an account with creative and status details."""
    try:
        data = await snap_request(f"adaccounts/{params.ad_account_id}/ads")
        ads = data.get("ads", [])
        if not ads:
            return "No ads found."
        lines = ["## Ads\n"]
...
async def snapchat_get_performance_stats(params: StatsInput) -> str:
    ...
    Args:
        ...
        params.granularity: TOTAL (default), DAY, or HOUR
    """
    try:
        ...
        query = {
            "fields": (
                ...
                "screen_time_millis,frequency,reach,conversion_purchases,"
                "conversion_purchases_value,total_installs"
            ),
            "granularity": params.granularity,
            "start_time":  f"{params.start_date}T00:00:00.000-00:00",
            "end_time":    f"{params.end_date}T23:59:59.000-00:00",
        }
...
async def snapchat_get_account_report(params: StatsInput) -> str:
    ...
    try:
        ...
        query = {
            "fields": (
                "impressions,swipes,spend,swipe_up_percent,video_views,"
                "conversion_purchases,conversion_purchases_value,reach,frequency"
            ),
            "granularity": params.granularity,
            "start_time":  f"{params.start_date}T00:00:00.000-00:00",
            "end_time":    f"{params.end_date}T23:59:59.000-00:00",
...
async def snapchat_update_status(params: UpdateStatusInput) -> str:
    ...
    try:
        ...
        # First fetch current entity to get full object
        single_map = {"campaigns": "campaign", "adsquads": "adsquad", "ads": "ad"}
        key = single_map.get(params.entity_type, params.entity_type.rstrip("s"))
        ...
        payload = {params.entity_type: [entity]}
        await snap_request(f"{params.entity_type}/{params.entity_id}", method="PUT", json=payload)
...
async def snapchat_get_creatives(params: AdAccountInput) -> str:
    """List all creatives (ad formats, media, headlines) in your ad account."""
    try:
        data = await snap_request(f"adaccounts/{params.ad_account_id}/creatives")
